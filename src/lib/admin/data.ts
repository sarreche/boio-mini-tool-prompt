...
export async function getDashboard(days: number) {
  ...
  const [users, executions, attempts, usage, requests, deletions, subscriptions, ratings, trials] = await Promise.all([
    admin.auth.admin.listUsers({ page: 1, perPage: 1 }),
    admin.from("executions").select("id,status,user_id,task_id,created_at", { count: "exact" }).gte("created_at", since),
    admin.from("execution_attempts").select("status,provider_code,model_code,input_tokens,output_tokens,latency_ms,created_at").gte("created_at", since),
    admin.from("usage_events").select("units,plan_id,billable,occurred_at").gte("occurred_at", since),
    ...
    admin.from("ratings").select("is_helpful").gte("created_at", since),
  ...
  return {
    ...
    uses: usage.data?.reduce((sum, item) => sum + item.units, 0) ?? 0,
    succeeded,
    failed,
