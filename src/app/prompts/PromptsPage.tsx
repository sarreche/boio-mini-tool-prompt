...
import { Archive, BookOpen, BowlFood, ChatCircle, CheckSquare, ClockCounterClockwise, Copy, EnvelopeSimple, Globe, House, Lightbulb, ListBullets, MagicWand, PencilSimple, SquaresFour, ThumbsDown, ThumbsUp, PaperPlaneTilt, Plus, SignOut, Sparkle, Toolbox, Translate, UserCircle, X } from "@phosphor-icons/react";
...
export default function PromptsPage({ initialLang }: { initialLang: Locale }) {
  ...
  function startNewChat(task: TaskCard | null = null) { setConversationId(null); setMessages([]); setSelectedTask(task); setInput(""); setError(""); }
  ...
  async function submitMessage(trimmed: string, regenerationOfExecutionId?: string) {
    ...
    try {
      const response = await fetch("/api/inference", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ input: trimmed, locale: lang, taskCode: selectedTask?.code, conversationId, regenerationOfExecutionId, clientRequestId: crypto.randomUUID() }), signal: controller.signal });
      ...
      <section className="min-w-0 px-4 pb-12 pt-4 sm:px-7 lg:px-10 xl:px-16">
        ...
        <div className="mx-auto mt-6 max-w-6xl">
          {messages.length === 0 ? <>
            ...
            <Composer t={t} input={input} setInput={setInput} busy={busy} selectedTask={selectedTask} tasks={data?.tasks ?? []} conversationId={conversationId} onSubmit={sendMessage} onArchive={archiveConversation} onSelectTask={setSelectedTask} onStop={stopGeneration} />
